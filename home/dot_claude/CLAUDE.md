...
These rules apply on every task. Topic-specific workflows are not loaded by default — invoke them as slash commands when relevant.
...
## Portability of Code and Docs

Any code, script, configuration, or documentation you write must work on
machines other than the one you're running on. Never bake in references
specific to the current machine — absolute home paths, the local username
or hostname, local IPs, locally-installed tool paths, or OS-specific
assumptions. Anything machine- or environment-specific must be parameterized.

**Why:** Artifacts written during a session absorb details discovered on
this machine (paths, ports, usernames) and then break silently for
teammates, in CI, or on a future machine. Hardcoded personal paths also
leak environment details into shared history.

**How to apply:**

- **Parameterize, don't hardcode.** Machine-specific values come from, in
  order of preference: environment variables with sensible defaults, CLI
  arguments, or a config file. Code reads them; it never embeds them.
- **Env/config files are the designated home for machine-specific values —
  paired with a committed template.** The local file (`.env`) holds real
  values and stays untracked; commit a `.env.example` / `config.sample.*`
  documenting every key with placeholder values.
- **In projects with a build step, classify every key in the template.**
  Group or annotate keys in `.env.example` along two axes: when the value
  is consumed (build time, runtime, or both — a build-time value is baked
  into the artifact and won't change without a rebuild) and whether it's a
  secret or exposed (for frontend builds, server-only vs client-bundle).
  Follow the framework's existing convention (`NEXT_PUBLIC_*`, `VITE_*`)
  rather than inventing a parallel naming scheme.
- **Secrets are runtime-only.** Never supply a secret as a build-time
  value — it gets baked into image layers or the client bundle. Inject
  secrets at runtime via the platform's secret mechanism.
- **Derive paths, don't spell them.** Use `~`/`$HOME` expansion
  (`Path.home()`, `os.path.expanduser`), XDG variables, or paths relative
  to the repo root or the script's own location — never `/home/<user>/...`
  literals.
- **Docs use placeholders.** Write `<project-root>`, `<your-api-key>`,
  `example.com` — not the actual local path, key, or internal hostname.
  Ports and URLs in examples are the project's documented defaults.
- **Don't over-parameterize.** The target is machine-specific values, not
  every constant. A genuinely universal value needs no knob (see Code
  Quality and Simplicity).
- **Verify before finishing.** Scan produced files for the current
  username, hostname, and home-directory path; any hit is a bug unless it
  falls under an exception.
- **Exceptions.** Dotfiles/system-config repos where machine specificity is
  the point (parameterize with the repo's own mechanism, e.g. chezmoi
  templates), and genuinely single-use scratch files that won't be
  committed or shared. When in doubt, parameterize.
...
When testing with Playwright (or similar browser automation), delegate the testing to a subagent instead of driving the browser from the main conversation, and pick the cheapest model that can handle the job.

**Why:** Browser automation produces verbose output (page snapshots, screenshots, network logs) that bloats the main context, and most test-execution steps are mechanical — they don't need the main model's reasoning. A faster, cheaper model in a fresh context does this work quicker at lower cost.

**How to apply:**

- **Choose the model by complexity.** `haiku` for simple checks (page loads, an element exists, a single form submits); `sonnet` for typical multi-step flows (login → navigate → interact → assert); `opus` only for genuinely complex workflows or intricate UI interaction where cheaper models are likely to get lost.
- **Write a fully self-contained brief.** The subagent starts with a fresh context and a less capable model — it knows nothing about this session and cannot infer intent. Spell out everything it needs: the URL and port, how to verify/start the dev server, credentials or test fixtures, the exact steps to perform, what "pass" and "fail" look like, and precisely what to report back (results per step, error messages, screenshot paths). Over-specify rather than under-specify.
- **Keep judgment in the main context.** The subagent executes and reports; interpreting an unexpected failure, deciding what to fix, or changing test scope stays with the main conversation.
